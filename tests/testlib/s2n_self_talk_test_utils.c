...
S2N_RESULT enable_recv_cb(struct s2n_connection *conn)
{
    ...
    RESULT_ENSURE(s2n_connection_is_ktls_enabled(conn, S2N_KTLS_MODE_RECV), S2N_ERR_SAFETY);
    return S2N_RESULT_OK;
}
...
S2N_RESULT enable_send_cb(struct s2n_connection *conn)
{
    ...
    RESULT_ENSURE(s2n_connection_is_ktls_enabled(conn, S2N_KTLS_MODE_SEND), S2N_ERR_SAFETY);
    return S2N_RESULT_OK;
}
