...
static S2N_RESULT start_client(int fd, int read_pipe)
{
    ...
    /* kTLS ENABLE */
    /* EXPECT_SUCCESS(s2n_connection_ktls_enable_recv(client_conn)); */
...

    {
        ...
        /* ------------ read alert */
        int bytes_read = s2n_recv(client_conn, recv_buffer, 2, &blocked);
        /* expect graceful shutdown */
        EXPECT_SUCCESS(bytes_read);
        {
            /* recv close_notify from  peer */
            EXPECT_EQUAL(bytes_read, 0);
            EXPECT_FALSE(client_conn->alert_sent);
            EXPECT_TRUE(s2n_atomic_flag_test(&client_conn->close_notify_received));
            ...
            /* send close_notify back to peer */
        ...
        /* ------------ round 2 */
        RESULT_GUARD_POSIX(read(read_pipe, &sync, 1));
        /* confirm already closed */
        bytes_read = s2n_recv(client_conn, recv_buffer, 1, &blocked);
        EXPECT_EQUAL(bytes_read, 0);
...
static S2N_RESULT start_server(int fd, int write_pipe)
{
    ...
    char send_buffer[2] = { 0 };
    {
        ...
        /* ------------ write alert */
        {
            int rc = s2n_shutdown(server_conn, &blocked);
            EXPECT_SUCCESS(rc);
            /* recv close_notify back from  peer */
            EXPECT_EQUAL(rc, 0);

            EXPECT_TRUE(server_conn->alert_sent);
            EXPECT_TRUE(s2n_atomic_flag_test(&server_conn->close_notify_received));
            EXPECT_FALSE(s2n_atomic_flag_test(&server_conn->write_closed));
