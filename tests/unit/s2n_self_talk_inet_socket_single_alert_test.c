...
S2N_RESULT helper_generate_test_data(struct s2n_blob *test_data_blob)
{
    ...
    EXPECT_SUCCESS(s2n_stuffer_init(&test_data_stuffer, test_data_blob));
...
S2N_RESULT start_client(int fd, int read_pipe, const struct self_talk_inet_socket_callbacks *socket_cb)
{
    ...
    uint8_t *test_data_ptr = test_data;
    {
        ...
        /* read record 1 */
        {
            ...
            /* confirm that entirety of SEND_LEN is as we expect */
        ...
        RESULT_GUARD_POSIX(read(read_pipe, &sync, 1));
        {
            ...
            read_len = s2n_recv(client_conn, recv_buffer, SEND_LEN + 11, &blocked);
            EXPECT_FAILURE_WITH_ERRNO(read_len, S2N_ERR_CLOSED);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    /* /1* server *1/ */
    /* { */
    /*     const struct self_talk_inet_socket_callbacks server_send_enable = { */
    /*         .s_post_handshake_cb = enable_send_cb, */
    /*         .c_post_handshake_cb = noop_cb, */
    /*     }; */
    /*     const struct self_talk_inet_socket_callbacks server_send_and_recv_enable = { */
    /*         .s_post_handshake_cb = enable_send_and_recv_cb, */
    /*         .c_post_handshake_cb = noop_cb, */
    /*     }; */
    /*     EXPECT_OK(launch_test(&server_send_enable)); */
    /*     EXPECT_OK(launch_test(&server_send_and_recv_enable)); */
    /* } */

    /* /1* client *1/ */
    /* { */
    /*     const struct self_talk_inet_socket_callbacks client_recv_enable = { */
    /*         .s_post_handshake_cb = noop_cb, */
    /*         .c_post_handshake_cb = enable_recv_cb, */
    /*     }; */
    /*     const struct self_talk_inet_socket_callbacks client_send_and_recv_enable = { */
    /*         .s_post_handshake_cb = noop_cb, */
    /*         .c_post_handshake_cb = enable_send_and_recv_cb, */
    /*     }; */
    /*     EXPECT_OK(launch_test(&client_recv_enable)); */
    /*     EXPECT_OK(launch_test(&client_send_and_recv_enable)); */
    /* } */

    /* /1* server and client *1/ */
    /* { */
    /*     const struct self_talk_inet_socket_callbacks server_and_client_send_and_recv_enable = { */
    /*         .s_post_handshake_cb = enable_send_and_recv_cb, */
    /*         .c_post_handshake_cb = enable_send_and_recv_cb, */
    /*     }; */
    /*     EXPECT_OK(launch_test(&server_and_client_send_and_recv_enable)); */
    /* } */
