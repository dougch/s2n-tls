...
static S2N_RESULT start_client(int fd, int read_pipe)
{
...

    {
        ...
        ssize_t result = -1;
        RESULT_GUARD_POSIX(s2n_recv(client_conn, recv_buffer, 2, &blocked));
        if (debug) {
            printf("\n========== start_client s2n_recv %c %c\n", CHAR_A, recv_buffer[0]);
        }
        ...
        /* expect NOT graceful shutdown */
        EXPECT_FAILURE_WITH_ERRNO(bytes_read, S2N_ERR_ALERT);

        {
            /* dont send close_notify on fatal */
        ...
        EXPECT_FAILURE_WITH_ERRNO(bytes_read, S2N_ERR_CLOSED);

        if (debug) {
            printf("========== start_client s2n_recv %c %c\n", CHAR_B, recv_buffer[0]);
        }
    }
