...
int s2n_process_alert_fragment(struct s2n_connection *conn)
{
    ...
    while (s2n_stuffer_data_available(&conn->in)) {
        ...
        printf("========== s2n_process_alert_fragment bytes_required: %d  data_avail: %d\n ", bytes_required, s2n_stuffer_data_available(&conn->in));
...
/* Evaluate adding support for sending warning-level alerts to kTLS before adding
 * more warning-level alert.
 *
 * Currently the only warning-level alert s2n-tls sends is for unsupported
 * renegotiation: S2N_TLS_ALERT_NO_RENEGOTIATION. Since this happens during the
 * handshake kTLS does not support sending warning-level alerts. */
